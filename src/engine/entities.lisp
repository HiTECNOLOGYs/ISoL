...
(defclass Door (Map-Element)
  ((display-character-open :initarg :display-character-open
                           :initform nil
                           :accessor display-character-open)
   (display-character-closed :initarg :display-character-closed
                             :initform #\+
                             :accessor display-character-closed)
   (open :initform nil
         :accessor open-p))
  (:documentation "Object that can be opened or closed by player."))

(defmethod initialize-instance :after ((instance Door) &rest initargs)
  (declare (ignore initargs))
  (setf (display-character instance) (if (open-p instance)
                                       (display-character-open instance)
                                       (display-character-closed instance))
        (passable-p instance) (open-p instance)))
